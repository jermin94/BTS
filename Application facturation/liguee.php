...
// En cas d'erreur, on affiche un message et on arrête tout
  die('Erreur : '.$e->getMessage());
    ...
    /* On supprime les information du client  dans la table ligue s'il ne s'enregistre pas*/
               $bdd->exec("DELETE FROM  ligue  WHERE nomlig = 'test_recup_incremt' ");
...
$tableau[] = $donnees['numCompte'];   /* on stock chaque resultat dans une variable tableau */
 ...
 /* on verifie si le code  client  n'a pas été modifié et s'il y a une correspondance dans la basse de donnée*/
$trouver=0;
for ($i=0; $i <(count($tableau)-1) && $trouver!=1  ; $i++) { 
 ...
 if( $trouver){ 
      suppression();
      $html = "<h3 style='color:red'>ERREUR : Enregistrement impossible le code client a été modifié ,evitez de plaissanter !!</h3>";
 }
       ...
       $html = "<h3 style='color:green'>BRAVO : Enregistrement Effectué </h3>"; 
        ...
        $html = "<h3 style='color:red'>ERREUR : Enregistrement impossible car il manque un ou plusieurs champs non renseignés </h3>";
 }
 ...
 echo $html;
    ...
    echo "<h3 style='color:red'>ERREUR : Enregistrement impossible le code client n'a été distribué </h3>";
}
?>
