...
$recherche = $bdd->query('SELECT * FROM dossiers_bkpe where id_partenaire="'.$idT.'" and categorie="pret" order by statut');
while ($donnees = $recherche->fetch())
{
	...
	{	
		...
		echo '</tr>'."\n";
		$reponse2 = $bdd->query('SELECT * FROM dossiers_bkpe where id_partenaire="'.$idT.'" and categorie="pret" order by statut');
